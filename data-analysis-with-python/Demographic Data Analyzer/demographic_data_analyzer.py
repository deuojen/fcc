...
def calculate_demographic_data(print_data=True):
    ...
    rich_percentage = round(
        100 * num_min_workers.loc[num_min_workers["salary"] == ">50K"].size / num_min_workers.size, 1)

    # num_min_workers = len(df.loc[(df['hours-per-week'] >= min_work_hours) & (df['salary'] == '>50K')])
    # rich_percentage = 100*num_min_workers/len(df)

    # What country has the highest percentage of people that earn >50K?
    highest_country = (df[["salary", "native-country"]].groupby("native-country").apply(
        lambda g: g.loc[g["salary"] == ">50K"].size / g.size * 100))
    ...
    # highest_earning_country_percentage = round(df.loc[df['salary']=='>50K','native-country'].value_counts(normalize = True)[0]*100, 10)
